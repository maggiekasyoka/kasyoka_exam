try {
    node {
        ...
        stage('Build docker image') {
            sh "docker build -t kasyoka:latest ."
        }
        stage('Docker login to hub and push the image') {
            sh "docker login -u 'maggiekasyoka' -p 'Ndambuki_01' "
            sh "docker tag kasyoka:latest maggiekasyoka/examrepo:latest"
            sh "docker push maggiekasyoka/examrepo:latest"
	    sh "docker run -d -p 7121:80/tcp kasyoka:latest"	
        }
